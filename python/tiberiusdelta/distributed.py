...
``tiberiusdelta`` does not require Spark; importing ``tiberiusdelta.distributed`` and
calling it does.
...
Usage::
    ...
    report = sync_tables_distributed(
        dbutils.secrets.get(scope="prod", key="sqlserver_connection_string"),
        "/Volumes/main/raw/mssql/",
        TABLES,
    )
    print(report["total_rows_fetched"], "rows across", len(report["tables"]), "tables")

Measure before reaching for this. A single worker syncs several hundred unchanged tables
in seconds, and a table's rows move at a few hundred thousand a second, so the
single-node path is sufficient for most workloads; see ``docs/operations.md``.
"""
...
def plan_partitions(table_count: int, default_parallelism: int, requested: int | None) -> int:
    ...
    one TDS login rather than one each.
...
def sync_tables_distributed(
    ...
    **sync_kwargs,
    ...
    Each table is synced exactly as :func:`tiberiusdelta.sync_tables` would sync it. The
    ...
    :param connection_string: As for :func:`tiberiusdelta.sync_tables`. Note that this is
        ...
        with it.
    :param output_uri: As for :func:`tiberiusdelta.sync_tables`.
    :param tables: One table configuration per table, as for
        :func:`tiberiusdelta.sync_tables`.
    ...
    :param sync_kwargs: Passed through to :func:`tiberiusdelta.sync_tables`, for example
    ...
    if "progress" in sync_kwargs:
        raise ValueError(
            ...
            "instead, or use tiberiusdelta.sync_tables for a single-node run."
        )
    ...
    if spark is None:
        ...
        if spark is None:
            raise RuntimeError(
                ...
                "tiberiusdelta.sync_tables for a single-node run"
            )
    ...
    collected = (
        context.parallelize(tables, partitions).mapPartitions(worker).collect()
    )
    ...
    if failures and raise_on_error:
        detail = "; ".join(
            f"{', '.join(f['tables'])}: {f['error']}" for f in failures
        )
        raise RuntimeError(
