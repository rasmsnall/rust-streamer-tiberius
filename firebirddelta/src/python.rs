...
//! `KeyboardInterrupt` between tables, and returns the report as a Python object. Mirrors
//! `tiberiusdelta::python` almost exactly; only the field additions `crate::types`'
//! Firebird-specific exclusion policy needs (`excluded_columns`, `ColumnPreflight`'s
//! `arrow_type` becoming optional and gaining `excluded`) differ.
//!
...
pub struct PyTableSyncStats {
    ...
    /// Columns excluded from the sync entirely: this crate's Firebird client cannot
    /// read them even as text (see `crate::types`'s module docs).
...
impl From<TableSyncStats> for PyTableSyncStats {
    fn from(s: TableSyncStats) -> Self {
        Self {
            ...
            excluded_columns: s.excluded_columns,
        }
    }
}
    ...
    fn __repr__(&self) -> String {
        format!(
            "ColumnPreflight(name={:?}, source_type={:?}, arrow_type={:?}, recognised={}, \
             excluded={})",
            self.name, self.source_type, self.arrow_type, self.recognised, self.excluded
        )
    }
}
...
pub struct PyTablePreflight {
    ...
    table: String,
    /// Every column the catalog reports for this table.
    columns: Vec<Py<PyColumnPreflight>>,
    /// False if the configured watermark column is missing or not selectable.
    watermark_present: bool,
    /// Configured primary key columns that are missing or not selectable.
...
fn build_config(
    ...
    // "/_firebirddelta_checkpoints" from nothing would probe the filesystem root instead.
