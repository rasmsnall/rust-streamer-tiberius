...
//!
//! The only asynchronous module in the crate, matching pgdelta's own `sink.rs`: delta-rs
//! exposes an async-only API, so a Tokio runtime exists, but it is confined to this
//! module and `crate::checkpoint`, which reuses [`upsert`] for its own tiny table.
//!
...
//! each table is synced independently, from an independent live query, and
//! `CLAUDE.md`'s own threat model already accepts that cross-table consistency is
//! weaker here than a single dump file's. So the atomic unit here is *one table's merge*,
//! not the whole run: `DeltaTable::merge` is itself one atomic Delta commit, and this
//! module does not attempt to batch several tables' merges into one transaction, because
//! sequencing "merge this table's data, then advance its checkpoint" per table, one
//! table at a time, is both simpler and safer than it looks. If the process crashes
//! between a table's merge committing and its checkpoint being written (see
//! `crate::checkpoint`), the checkpoint stays at the old value and the next run
//! re-fetches and re-applies the same rows; because `MERGE` is an upsert, re-applying an
//! already-merged row is idempotent, not a duplicate. The reverse order (checkpoint
//! before merge) would not be safe: a crash between them would permanently skip rows
//! that were never actually merged. This module deliberately never advances a checkpoint
//! itself; the caller (`crate::pipeline`) must call `crate::checkpoint::advance` only
//! after `upsert` here has returned successfully.
...
mod tests {
    ...
    fn tmpdir(tag: &str) -> std::path::PathBuf {
        let dir =
            std::env::temp_dir().join(format!("tiberiusdelta-merge-{tag}-{}", std::process::id()));
