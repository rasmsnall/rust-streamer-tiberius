...
//!
//! Plays the same role pgdelta's `_pgdelta_loads` manifest plays for its own history, but
//! is read as well as written, since incremental sync needs to know where it left off.
//!
//! # One Delta table per source table
//!
//! Checkpoints live at `<checkpoint_uri>/<schema>/<table>`, mirroring the layout of the
//! data tables themselves, each holding the single current row for its own source table.
//!
...
//!
//! The cost is that `SELECT * FROM _streamer_checkpoints` no longer shows everything at
//! once. Each checkpoint still carries its own `table_name`, so a union view over the
...
/// Qualified source table name. Redundant with the checkpoint's own path, and kept so a
/// union view over the prefix is self-describing.
...
mod tests {
    ...
    fn reading_a_checkpoint_that_does_not_exist_yet_is_none_not_an_error() {
        ...
        rt().block_on(async {
            let found = read(&uri_for(&uri(&dir), "dbo/customers")).await.unwrap();
    ...
    fn advance_then_read_round_trips() {
        ...
        rt().block_on(async {
            let at = uri_for(&uri(&dir), "dbo/customers");
            advance(&at, "dbo.customers", "updated_at", "2026-01-03 12:00:00", 0)
                .await
                .unwrap();
            assert_eq!(
    ...
    fn advancing_the_same_table_again_replaces_its_checkpoint() {
        ...
        rt().block_on(async {
            let at = uri_for(&uri(&dir), "dbo/customers");
            advance(&at, "dbo.customers", "updated_at", "v1", 0)
                .await
                .unwrap();
            advance(&at, "dbo.customers", "updated_at", "v2", 1)
                .await
                .unwrap();
    ...
    fn each_table_gets_its_own_checkpoint_location() {
        let prefix = "file:///out/_streamer_checkpoints";
        let customers = uri_for(prefix, "dbo/customers");
        let orders = uri_for(prefix, "dbo/orders");
        assert_ne!(customers, orders);
        assert_eq!(customers, "file:///out/_streamer_checkpoints/dbo/customers");
        ...
        assert_eq!(
            uri_for("file:///out/_c/", "dbo/customers"),
            customers.replace("_streamer_checkpoints", "_c")
        );
    }
    ...
    fn multiple_tables_are_tracked_independently() {
        ...
        rt().block_on(async {
            ...
            let customers = uri_for(&prefix, "dbo/customers");
            let orders = uri_for(&prefix, "dbo/orders");
            advance(&customers, "dbo.customers", "updated_at", "v1", 0)
                .await
                .unwrap();
            advance(&orders, "dbo.orders", "id", "100", 0)
                .await
                .unwrap();
